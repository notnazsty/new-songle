...
import { UsersSpotifyTokenData, UserSpotifyProfileData } from '../models/firebase/account'
import { SpotifyTokenResponse, SpotifyProfileData } from '../models/spotify/user'
...
const Callback: NextPage = () => {
  ...
  useEffect(() => {
    if (user && userCode) {
      ...
      fetch("/api/initialSpotifyAuth?code=" + encodeURI(userCode)).then(async (data) => {
        ...
        if ("refresh_token" in tokenData) {
          ...
          await setDoc(doc(userRef, user.uid), { usersSpotifyTokenData: spotifyTokenData }, { merge: true });


          const profileData = await getSpotifyData(tknData.access_token) as SpotifyProfileData;
          ...
          await setDoc(doc(userRef, user.uid), { spotifyConnected: true, spotifyProfileData: uploadProfileData }, { merge: true })
